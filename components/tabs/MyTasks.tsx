...
function TaskRow({ task, onToggle, onDelete, projects }: { task: UserTask; onToggle: () => void; onDelete: () => void; projects: Project[] }) {
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.03)', border: '0.5px solid rgba(255,255,255,0.07)', borderLeft: `2px solid ${tc}`, borderRadius: '0 10px 10px 0', padding: '11px 14px', display: 'flex', alignItems: 'center', gap: 12, opacity: task.done ? 0.5 : 1 }}>
      ...
      {task.done && <button onClick={onToggle} style={{ background: 'transparent', border: '0.5px solid rgba(255,255,255,0.12)', borderRadius: 4, color: 'rgba(255,255,255,0.3)', padding: '2px 8px', cursor: 'pointer', fontSize: 11 }}>Undo</button>}
...
export default function MyTasks({ profile, tasks, onToggle, onDelete, onAdd, projects }: {
  ...
  const pending = tasks.filter(t => !t.done).sort((a, b) => sort === 'priority' ? a.priority - b.priority : (a.date || '9999').localeCompare(b.date || '9999'))
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7, marginBottom: 16 }}>
        ...
        {pending.map(t => <TaskRow key={t.id} task={t} onToggle={() => onToggle(t.id)} onDelete={() => onDelete(t.id)} projects={projects} />)}
      </div>
      {adding ? (
      ...
      {done.length > 0 && (
        <div style={{ marginTop: 28 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
            {done.map(t => <TaskRow key={t.id} task={t} onToggle={() => onToggle(t.id)} onDelete={() => onDelete(t.id)} projects={projects} />)}
          </div>
        </div>
      )}
    </div>
